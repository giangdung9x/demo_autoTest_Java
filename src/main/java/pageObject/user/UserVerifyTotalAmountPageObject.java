...
public class UserVerifyTotalAmountPageObject extends BasePage{

	private WebDriver driver;

	public UserVerifyTotalAmountPageObject(WebDriver driver) {
		this.driver = driver;
	}
	...
	//------------------------------
	//ADMIN PAGE
	public void openTabAdminPage() {
		((JavascriptExecutor)driver).executeScript("window.open('about:blank', '-blank')");
		ArrayList<String> tabs = new ArrayList<String>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(1));
		driver.get("https://" + "SS15243" + ":" + "12345" + "@showslinger-staging.herokuapp.com/admin");
	}

	@Step("Verify text - Name of screen - {0}")
	public boolean isNameOfScreen(String nameOfScreen) {
		waitForElementVisible(driver, VerifyTotalAmountPageUI.DASHBOARD_TITLE, nameOfScreen);
		return isElementDisplayed(driver, VerifyTotalAmountPageUI.DASHBOARD_TITLE, nameOfScreen);
	}

	@Step("Click to button - {0}")
	public void clickToButton(String buttonName) {
		waitForElementClickable(driver,VerifyTotalAmountPageUI.BUTTON_VENUES, buttonName);
		clickToElement(driver, VerifyTotalAmountPageUI.BUTTON_VENUES, buttonName);
	}

	@Step("input value - {0}")
	public void inputValueVenueName(String nameVenue) {
		waitForElementVisible(driver, VerifyTotalAmountPageUI.INPUT_SEARCH_NAME);
